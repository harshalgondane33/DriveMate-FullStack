  ...
  return  car? (
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 mt-16'>
      <button onClick={()=> navigate(-1)} className='flex items-center gap-2 mb-6 text-gray-600 cursor-pointer'>
        <img src={assets.arrow_icon} alt='arrow-back' className='rotate-180 opacity-65' /> 
        Back to All Cars
      </button>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12'>
          ...
          <div className='space-y-6'>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
              {[
                  ...
                  <div key={text} className='flex flex-col items-center bg-light p-2 rounded-lg'>
            ...
            <ul className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
              {
                ["360 Camera","Bluetooth","GPS","Heated Seats","Rear View Camera"].map((item)=>(
                  <li key={item} className='flex items-center text-gray-500'>
                    <img src={assets.check_icon} className='h-4 mr-2' alt='icon'/>
                    {item}
                  </li>
                ))
              }
        ...
        <form onSubmit={handleSubmit} className='shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500'>
          <p className='flex items-center justify-between test-2xl text-gray-800
